...
# Convolution with clamped weights.
...
# For input of RGBD images and speeds. 
...
# For prediction of RGBD images and speeds. 
...
# Multiple Timescale RNN with Gates. 
...
class Forward(nn.Module): 
    ...
    # Predict observations based on prior or posterior inner-states.
    ...
    # Processing an entire batch of episodes, step by step.
    def forward(self, prev_action, rgbd, speed):
        ...
        for step in range(steps-1):
            ...
            zq_mu_list.append(zq_mu) ; zq_std_list.append(zq_std) ; hq_list.append(hq)
        ...
        hqs = torch.cat(hq_list, dim = 1)
        zp_mus  = torch.cat(zp_mu_list,  dim = 1)
        zp_stds = torch.cat(zp_std_list, dim = 1)
        zq_mus  = torch.cat(zq_mu_list,  dim = 1)
        zq_stds = torch.cat(zq_std_list, dim = 1)
        pred_rgbd = torch.cat(zq_rgbd_pred_list,  dim = 1)
        pred_spe  = torch.cat(zq_speed_pred_list, dim = 1)
        return(
            (zp_mus, zp_stds), 
            (zq_mus, zq_stds, pred_rgbd, pred_spe, hqs))
